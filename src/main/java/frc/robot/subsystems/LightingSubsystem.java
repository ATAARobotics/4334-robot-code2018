...
public class LightingSubsystem extends SubsystemBase {
    ...
    public LightingSubsystem() {
        ...
        generateGradient(5, new int[] {255, 0, 0}, new int[] {0, 255, 0}, new int[] {0, 0, 255});

        candle.setLEDs(0, 0, 0);
    }

    private void generateGradient(int step, int[]... colors) {
        colorList = new int[3][step * colors.length];
        for (int color = 0; color < colors.length; color++) {
            ...
            for (int i = 0; i < step; i++) {
                colorList[color + i] = new int[] {(int) distance[0] * (i / step), (int) distance[1] * (i / step), (int) distance[2] * (i / step)};
            }
        }
    }
    ...
    public void lightUp(double lightPercent) {
        ...
        for (int i = 0; i < count-2; i++) {
            candle.setLEDs(colorList[(i-offset+colorList.length) % colorList.length][0], colorList[(i-offset+colorList.length) % colorList.length][1], colorList[(i-offset+colorList.length) % colorList.length][2], 0, i+8, 1);
            candle.setLEDs(colorList[(i-offset+colorList.length) % colorList.length][0], colorList[(i-offset+colorList.length) % colorList.length][1], colorList[(i-offset+colorList.length) % colorList.length][2], 0, 127-i, 1);
        }
    }
}